...
struct SuggestionSettingsData: Equatable {
    ...
    /// When on (the default), ghost text is revealed token-by-token as the model decodes, and each
    /// partial is an acceptable session the user can Tab into early — the first word appears
    /// without waiting for the whole generation. When off, the suggestion appears once, fully
    /// formed, after generation finishes.
