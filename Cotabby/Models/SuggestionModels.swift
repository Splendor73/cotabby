...
struct SuggestionRequest: Equatable, Sendable {
    ...
    /// Hard ceiling on shown words, from the user's length setting. `completionLengthInstruction`
    /// is guidance the model may ignore; this is the enforcement, applied by the normalizer at
    /// word starts. `nil` (test fixtures, legacy call sites) means no trim.
    let maxCompletionWords: Int?
